...
def run_analysis(args: Namespace):
    ...
    image_ids = [ int(os.path.splitext(os.path.basename(img_path))[0]) for img_path, _ in dataset.samples ]
    ...
    # Load part locations
    log('Loading part locations...')
    part_locs = pd.read_csv(os.path.join(os.path.dirname(dataset_split_path), 'part_locs.csv'))
    part_locs = part_locs[part_locs.image_id.isin(image_ids)]
    for img_id, (img_width, img_height) in zip(image_ids, image_sizes):
    ...
    assert np.all(part_locs[['x', 'y']] <= img_resize), 'Part locations are outside of image boundaries'
    ...
    for class_name in dataset.classes:
        ...
        save_alignment_matrix(os.path.join(class_dir, f'alignment_matrix.png'), alignment_matrix)
    logclose()
