...
local_parser.add_argument('--top_classes', type=int, default=10, help='number of most activated classes for which display the top prototypes (default: %(default)s)')
local_parser.set_defaults(func=local_analysis.run_analysis)


# Alignemnt scores arguments
alignment_parser = subparsers.add_parser('alignment', help='run alignemnt score analysis')
alignment_parser.add_argument('--img', type=str, required=True, help='path of an image in the dataset to use for evaluation')
alignment_parser.set_defaults(func=alignment_analysis.run_analysis)
